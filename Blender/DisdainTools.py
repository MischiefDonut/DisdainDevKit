...
class DisdainToolsOperator(bpy.types.Operator):
    ...
    def generate_a_run_speeds(self, scene, targ, filepath, start_frame = 0,
        end_frame = 0):
        ...
        if targ_obj.type != 'EMPTY':
            self.report({'ERROR_INVALID_INPUT'}, \
                "Target must be an Empty object!")
            return
...
class DisdainToolsPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = l.row()
        row.operator("render.disdaintools_operator", \
            text = "Generate A_Run() Speeds", icon='RENDER_ANIMATION')
...
def register():
    ...
    bpy.types.Scene.disdaintools = bpy.props.PointerProperty\
    (
        type = DisdainToolsSettings
    )
