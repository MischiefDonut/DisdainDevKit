...
def collectMeshes(context, bones, scale, matfun, useskel = True, usecol = False, usemods = False, filetype = 'IQM'):
    ...
    for obj in objs:
        if obj.type == 'MESH':
            data = obj.to_mesh(context.scene, usemods, 'PREVIEW')
...
class ExportIQM(bpy.types.Operator, bpy_extras.io_utils.ExportHelper):
    ...
    def execute(self, context):
        ...
        exportIQM(context, self.properties.filepath, self.properties.usemesh, False, self.properties.useskel, self.properties.usebbox, self.properties.usecol, self.properties.usescale, self.properties.animspec, matfun, self.properties.derigify, self.properties.boneorder)
        return {'FINISHED'}
